...
def train_nh(train, dev, path_to_save, used_model, lr=0.1, num_epochs=10, batch_size=10, dataset_name='default'):
    ...
    if used_model:
        ...
        model = NeuralHawkes(n_types=type_size, lr=lr, device=device)
    ...
    prefix = f'{prefix}_{dataset_name}_{NeuralHawkes.__name__}'
    ...
    model_path = path_to_save + f"{prefix}_model.pt"
    save_path = path_to_save + prefix

    return (save_path, model_path)

def train_causal_nh(train, dev, A, W, path_to_save, used_model, lr=0.1, num_epochs=10, batch_size=10,
                    dataset_name='default', model=CausalNeuralHawkesTrainableWeighted):
    now = str(datetime.datetime.today()).split()
    now = now[0] + "-" + now[1][:5]
    print("Processing data...")
    ...
    if used_model:
        ...
        model = model(A=A, W=W, n_types=type_size, lr=lr, device=device)
    ...
    prefix = datetime.datetime.now().strftime('%Y_%m_%H_%M_%S')
    prefix = f'{prefix}_{dataset_name}_{model_name}'
    t3 = time.time()
