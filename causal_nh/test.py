...
def test_prediction(test_data, idx_test_series, n_samples, start_test_idx, path_to_initialization, dataset_name):
    # read the model and simulate durations for integral
    path_to_save = path_to_initialization + '_test_prediction_original_NH_'
    path_to_model = path_to_initialization + '_model.pt'
    ...
    figure.tight_layout()
    plt.savefig(path_to_save + f"{dataset_name}_result.png")
    plt.show()
    ...
    plt.savefig(path_to_save + f"{dataset_name}_result_prediction.png")
    plt.show()

    return original_time, original_type, estimated_times, estimated_types, estimated_intensities
