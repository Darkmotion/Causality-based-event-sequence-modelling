import torch
from causal_nh.model import ContTimeLSTM_Cell
...
class CausalNeuralHawkes(nn.Module):
    def __init__(self, n_types, A, beta=0.1, hid_dim=32, lr=0.01):
        ...
        super(CausalNeuralHawkes, self).__init__()
        self.emb = nn.Embedding(self.n_types + 1, self.hid_dim)
        self.lstm_cell = ContTimeLSTM_Cell.CTLSTMCell(hid_dim, beta)
        self.hidden_lambda = nn.Linear(self.hid_dim, self.n_types)
        self.weighted_A = torch.nn.Parameter(torch.FloatTensor(self.A), requires_grad=True)
    ...
    def conttime_loss(
        ...
        part_one_likelihood = torch.zeros(batch_size)
        sum_likelihood = torch.zeros(batch_size)
        type_intensity = torch.nn.functional.softplus(torch.matmul(self.hidden_lambda(h), self.A.mul(self.weighted_A))).transpose(
            0, 1
        )
        for idx in range(batch_size):
            event_seq = event_seqs[idx]
...
class CausalNeuralHawkes_v2(nn.Module):
    def __init__(self, n_types, A, beta=0.1, hid_dim=32, lr=0.01):
        self.n_types = n_types
        self.beta = beta
        self.hid_dim = hid_dim
        ...
        super(CausalNeuralHawkes_v2, self).__init__()
        self.emb = nn.Embedding(self.n_types + 1, self.hid_dim)
        self.lstm_cell = ContTimeLSTM_Cell.CTLSTMCell(hid_dim, beta)
        self.hidden_lambda = nn.Linear(self.hid_dim, self.n_types)
    ...
    def conttime_loss(
        ...
        part_one_likelihood = torch.zeros(batch_size)
        sum_likelihood = torch.zeros(batch_size)
        type_intensity = torch.nn.functional.softplus(torch.matmul(self.hidden_lambda(h), self.A)).transpose(
            0, 1
        )
        for idx in range(batch_size):
            event_seq = event_seqs[idx]
        ...
        h_sim_list = torch.stack(h_sim_list)
        sim_intensity = torch.nn.functional.softplus(
            torch.matmul(self.hidden_lambda(h_sim_list), self.A)
        ).transpose(0, 1)
        part_two_likelihood = torch.zeros(batch_size)
        for idx in range(batch_size):
            coefficient = total_time_list[idx] / sim_len
